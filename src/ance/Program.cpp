...
namespace ance
{
    namespace
    {
        ...
        utility::Optional<Arguments> parseArguments(std::ostream& out, int const argc, char** argv)
        {
            Arguments arguments;
        ...
        int run(std::ostream& program_out, std::ostream& compiler_out, Arguments const& arguments)
        {
            ...
            if (compiler.compile(file_name)) exit_code = EXIT_SUCCESS;

            reporter.report();

            return exit_code;

            // todo: do function (with expansion explanation to lambda), then do declarations, then do type expression in general, then proceed from arguments_duplicate
            ...
            // todo: ideally, every compiler report error is caused at least once
            ...
            // todo: but think about this a bit more, see https://en.wikipedia.org/wiki/Uniform_function_call_syntax
...
int ance::run(std::ostream& program_out, std::ostream& compiler_out, int const argc, char** argv)
{
    ...
    try
    {
        ...
        return run(program_out, compiler_out, arguments.value());
#if defined(ANCE_HANDLE_ERRORS)
    }
    catch (std::exception const& e)
    {
        core::Reporter::printCritical(compiler_out, e.what());
        return EXIT_FAILURE;
    }
#endif
}
